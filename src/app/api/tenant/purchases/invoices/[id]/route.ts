...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const db = prisma as any;
    const inv = await db.supplierInvoice.findFirst({ where: { id: params.id, tenantId }, include: { lines: true, payments: true } });
    if (!inv) return NextResponse.json({ error: 'Introuvable' }, { status: 404 });
    return NextResponse.json(inv);
  } catch (e) {
    console.error('GET /api/tenant/purchases/invoices/[id]', e);
